    ...
    def execute(self, uid, *params):
        if len(params) == 0:
            ...
            if command in self.service.commands:
                ...
                self.service.msg(uid, '해당 명령이 존재하지 않아 도움말을 찾을 수 없습니다. "/msg {0} 도움말"을 입력해보세요.', self.service.name)
                return
        self.service.msg(uid, '==== {0} 도움말 ===='.format(target.name))
        if isinstance(target, Command):
            self.service.msg(uid, '사용법: {0} {1}'.format(target.name, target.syntax))
