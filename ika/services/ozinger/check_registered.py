...
class CheckRegistered(Listener):
    def check(self, uid, name):
        ...
        if nick and (nick.user or nick.user_alias):
            self.service.msg(uid, '이 닉네임은 이미 오징어 IRC 네트워크에 등록되어 있는 닉네임입니다. 계정의 주인이시라면 \x02/msg {} 로그인\x02 을, 주인이 아니시라면 지금 바로 닉네임을 다른 것으로 바꿔 주세요.', self.service.name)

    @asyncio.coroutine
    def NICK(self, uid, *params):
        self.check(uid, params[0])
